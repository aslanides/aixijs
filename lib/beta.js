class BetaModel {
	...
	update(a, e) {
		...
		pos.pr.update(rew == config.rewards.chocolate);
		pos.freq = pos.pr.mean();
		this.weights[pos.y * this.M + pos.x] = pos.freq;
	}
	...
	load() {
		...
		for (let i = 0; i < this.M; i++) {
			for (let j = 0; j < this.N; j++) {
				this.env.grid[i][j].freq = this.weights[i * this.M + j];
			}
		}
	}
}
