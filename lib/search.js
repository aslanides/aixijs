class Search {
    static playout(agent, playout_len) {
        ...
        for (var i = 0; i < playout_len; i++) {
            ...
            agent.model.do(action)
            reward += agent.model.generatePercept().rew
        }
        return reward
    }
}
...
class ExpectimaxTree {
    constructor() {
        this.root = new RLDecisionNode()
    }
    pruneAllBut(action) {
        // TODO
    }
    sample(agent) {
        this.root.sample(agent,0)
    }
    bestAction(agent) {
        return this.root.bestAction(agent)
    }
}
...
class ChanceNode extends SearchNode {
    ...
    sample(agent,dfr) {
        ...
        if (dfr > agent.horizon) {
            ...
            agent.model.do(this.action)
            var percept = agent.model.generatePercept()
            if (this.getChild(percept) == undefined) {
                this.addChild(percept)
            }
            reward = agent.utility(percept) + this.getChild(percept).sample(agent,dfr+1)
        }
        this.mean = (1 / (this.visits + 1)) * (reward + this.visits * this.mean)
        this.visits++
        return reward
