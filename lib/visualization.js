class Visualization {
	...
	//(currently storing in environment grid)
	//TODO Tidy up
	//TODO make arrows only visible for valid tiles
	constructor(env,trace) {
		...
		//(maybe put in rectangles to visualise in this way as well, similar to karpathy)
	...
	initGrid(){
		var svg = d3.select(gridvis).append('svg').attr('width', this.width).attr('height', this.height)
	...
	updateArrows(){
		for (var i = 0; i < this.grid.M; i++) {
			...
			for(var a=0;a<4;a++){
				...
				if (line_size > 5) //Only show arrows if line at least 3 pixels long
				{
