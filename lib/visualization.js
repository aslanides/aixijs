class Visualization {
	//We should get an array of slices, label of what each value is and description
	...
	draw() { // TODO May need to add parameters later for different environments (e.g. what states are visible)
		...
		for (var i = 0; i < this.grid.M; i++) {
			...
			//draw agent based on pos. (TODO generalise)
			this.context.fillStyle = c_agent;
			this.context.fillRect(this.trace[this.time].pos.x*(d+1),this.trace[this.time].pos.y*(d+1),d,d);
			this.drawToTile(this.trace[this.time].pos.x*this.d, this.trace[this.time].pos.y*this.d, this.trace[this.time].q)
	}


	//Draw a graph, given x and y axis and a title
	drawGraph(x, y, title){
		//TODO, good library below
		//http://canvasjs.com/editor/?id=http://canvasjs.com/example/gallery/dynamic/realtime_line/
	}

	//Draw data to a tile location (e.g. Q- Values)
	drawToTile(tile_x, tile_y, data){
			this.context.fillStyle = "white"
			this.context.font = "10px Arial"
			this.context.fillText(data, tile_x + this.d/4 , tile_y + this.d/2)
	}

	updateUI() {
		document.getElementById("select_time").value = this.time
		document.getElementById("r_ave").value = this.trace[this.time].reward / this.time
	}
