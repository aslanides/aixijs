class Visualization {
	constructor(env,trace) {
		...
		if (document.body.children[0].id == "canvas") {
			document.body.replaceChild(this.canvas,document.body.childNodes[0])
		} else {
			document.body.insertBefore(this.canvas,document.body.childNodes[0])
		}
	    this.context = this.canvas.getContext("2d")
		this.jumpTo(0)
		drawPlot(this.rew_trace,"average")
	}
	run(speed) {
		this.pause()
		var f = function(viz) {
	...
	updateUI() {
		...
		document.getElementById("r_ave").value = this.rew_trace[this.time] / this.time
