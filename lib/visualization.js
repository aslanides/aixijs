class Visualization {
	...
	action_map() { //maps action to envt specific move
...
class BayesGridVis extends GridVisualization {
	constructor(env, trace) {
		...
		this.plan_trace = trace.plan_trace;

		this.svg
			.attr('height', this.height + this.d + 70); //TODO fix magic no.

		this.svg
			.append('text')
			.text('Current Plan:')
			.attr('y', this.d * this.N + 30); //TODO font size etc.
	...
	updateAgent() {
		...
		for (let i = 0; i < this.plan_trace[this.time].length; i++) {

			let plan_i = this.svg.append('text')
				.text(this.plan_trace[this.time - 1][i])
				.attr('y', this.d * this.N + 80)
				.attr('x', this.d * i);
			//.attr('visibility', 'hidden');

			this.plan_vis.push(plan_i);
		}
	}
	...
	posteriorColor(tile, time) {
		...
		return {
			...
			b: 255 - p * this.color_normalization,
		};
	}
}

class BetaBayesVis extends BayesGridVis {
	constructor(env, trace) {
		super(env, trace);
		this.color_normalization = 255;
	}
}

class DirichletVis extends BetaBayesVis {
	constructor(env, trace) {
		super(env, trace);
	...
	posteriorColor(tile, time) {
		...
		let as = Util.sum(alphas) || 1;
		return {
			r: 255 - alphas[2] / as * this.color_normalization,
			g: 255,
			b: 255 - alphas[0] / as * this.color_normalization,
		};
	}
}
...
class TIVis extends Visualization {
	constructor(env, trace) {
		...
		let ti_counter = 0;
		for (let i = 0; i < this.plan_trace.length - 2; i++) {
	...
	updateAgent() {
		...
		for (let i = 0; i < this.plan_trace[this.time].length; i++) {
			let plan_i = this.svg.append('text')
				.text(this.plan_trace[this.time + 1][i])  //index off, needs to be a time step higher
