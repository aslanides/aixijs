class Visualization {
	constructor(env,trace) {
		...
		for (var i = 0; i < env.grid.M; i++) {
			this.rectangles.push(new Array(env.grid.N))
		}
	...
	initGrid(){
		...
		this.grid.tiles.forEach((row,idx) => {
...
class TabularVis extends Visualization {
	constructor(env,trace) {
		...
		this.arrows = []
		for (var i = 0; i < env.grid.M; i++) {
			this.arrows.push(new Array(env.grid.N))
		}
...
class BayesVis extends Visualization {
	constructor(env, trace) {
		super(env, trace)
		this.weights = []
		this.color_normalization = this.grid.M*this.grid.N*100
	...
	initModel() {
		this.grid.tiles.forEach(row => {
			row.forEach(tile => {
				tile.info = this.model_trace[0][tile.x * this.grid.N + tile.y]
	...
	updateModel() {
		this.grid.tiles.forEach(row => {
			row.forEach(tile => {
				tile.info = this.model_trace[this.time][tile.x * this.grid.N + tile.y]
...
class AIXIVis extends BayesVis {
	constructor(env,trace) {
		super(env,trace)
		this.jumpTo(0)
...
class ThompsonVis extends BayesVis {
	constructor(env,trace) {
		super(env,trace)
