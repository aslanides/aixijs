	...
	updateAgent() {
		this.grid.forEach(row => {
			row.forEach(tile => {
				...
				rectangle.attr('fill', `rgb(${Math.floor(c.r)},${Math.floor(c.g)},${Math.floor(c.b)})`);
			});
		});
	}
...
class DirichletVis extends BetaBayesVis {
	...
	posteriorColor(tile, time) {
		...
		return {
			r: 255 - alphas[2] * this.color_normalization,
			g: 255 - alphas[1] * this.color_normalization,
			b: 255 - alphas[0] * this.color_normalization,
		};
	}
