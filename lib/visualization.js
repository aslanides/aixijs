...
class BayesVis extends Visualization {
	...
	initModel(){
		for(var i = 0; i< this.grid.M; i++){
			for(var j = 0; j < this.grid.N; j++){
				//initialise weights (currently assumes uniform prior)
				this.grid.tiles[i][j].info = (1 / (this.grid.M * this.grid.N))
			}
		}
	}
	...
	updateModel(){
		for (var i = 0; i < this.grid.M; i++) {
			for (var j = 0; j < this.grid.N; j++) {
				...
				//update weights here
				this.grid.tiles[i][j].info = this.model_trace[this.time][i*this.grid.N + j]

				var r=255,g=255,b=255;
				var rectangle = this.rectangles[i*this.grid.N + j]
				var prob = this.grid.tiles[i][j].info
				var color_constant = 1000; //Seems to look ok
