...
class MDPVis extends Visualization{
	constructor(env, trace){
		...
		for (var i = 0; i < env.states.length; i++){
			this.state_circles[i] = env.config.states[i].pos
		}
		...
		for(var i = 0 ; i < this.state_circles.length; i++){
			var x = this.state_circles[i].x
			var y = this.state_circles[i].y
			this.svg.append('circle')
				.attr('cx', x)
				.attr('cy', y)
				.attr('r', 30)
		...
		//Initialise agent
		this.svg.append('circle')
			.attr('cx', this.state_circles[this.current_state_no].x)
			.attr('cy', this.state_circles[this.current_state_no].y)
			.attr('r', 15)
			.attr('fill', "blue")
			.attr('stroke', '#000')
			.attr('id', 'cpos')
		...
		//TODO init environment (circles, arrows, starting pos)
		//Need a set of arrows corresponding to each different action from each state
		//Arrows for a particular action have thickness ~ transition prob.
		//Maybe differentiate actions by colour?
		//Maybe only show arrows for current state to reduce clutter?
		//Perhaps need the state circle centres specified in config file? More flexibility in range of demos this way
	}
	...
	updateEnv(){
		...
		console.log("test")
		var x = this.state_circles[this.pos_trace[this.time]].x
		var y = this.state_circles[this.pos_trace[this.time]].y
		d3.select('#cpos')
        	.attr('cx', x)
        	.attr('cy', y)
	}
