class Options {
	constructor() {
		...
		//Usage: discount_changes = times when to change, discounts = what discount to use
		this.discount_changes = [0];
		this.discounts = [new GeometricDiscount(1)];


		//discount 6 ahead to plan for high reward, then change mind right before and discount only 1 to waste
	/*	for (let i = 0; i < 200; i++){
			this.discount_changes[i] = i;
			if (i % 5 == 0){
			this.discounts.push(new ConstantHorizonDiscount(200))
			}
			else this.discounts.push(new ConstantHorizonDiscount(200))
		}*/
	//	this.discounts = [new ];
	//	this.discount_changes = [0];
