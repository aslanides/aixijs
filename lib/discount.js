class Discount {
	...
	getValue(time, dfr){
		...
		if (time == this.times[this.current_discount_index]) {
			this.current_discount = this.discount_array[this.current_discount_index];
			this.current_discount_index++;	//relies on this function being called with monotonically increasing time values
		}
