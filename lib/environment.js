...
class IteratedPrisonersDilemma extends Environment {
	...
	makeModelClass(kind) {
		let opponents = this.config.opponents;
		let modelClass = [];
		let cfg = Util.deepCopy(this.config);
		for (let op in config.opponents) {
			cfg.opponent = config.opponents[op];
...
class Gridworld extends Environment {
	constructor(cfg) {
		...
		for (let i = 0; i < this.N; i++) {
			...
			for (let j = 0; j < this.M; j++) {
				...
				this.grid[i][j] = Tile.new(i, j, cfg, cfg.map[j][i]);
			}
		}
	...
	static proposeRandom(cfg) {
		...
		for (let i = 0; i < conf.goals.length; i++) {
			let gx = Util.randi(Math.floor(M / 2), M);
			let gy = Util.randi(Math.floor(N / 2), N);
			conf.goals[i].pos.x = gx;
			conf.goals[i].pos.y = gy;
			conf.map[gy][gx] = 'F';
		}
	...
	static isSolvable(env) {
		...
		while (pos < queue.length) {
			...
			for (let t of ptr.connexions) {
				...
				if (t.constructor == Dispenser || t.constructor == Chocolate) {
					return true;
				}
	...
	conditionalDistribution(e) {
		...
		if (e.obs != p.obs) {
			...
			if (rew == config.rewards.chocolate) {
				...
				return rew == config.rewards.wall && this.wall_hit;
			}
		}
	}
