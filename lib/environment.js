...
class BasicMDP extends Environment {
	// TODO
	constructor(config){
		this.states = []; //Will need a state obj similar to tile
		//TODO: State will need : transition probabilities (which state to change to and with what probability)
		// 		Reward, and an index (pos)
		
		this.current_state = config.initial_pos;
	}
	do(action)}{
		 //TODO Get the transition from the states internals, and update current state, determine reward etc.
	}
	generatePercept() {
        return {
            obs : this.current_state; //As we are assuming a fully observable MDP
            rew : this.reward;
        }
    }
