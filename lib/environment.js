...
class BasicMDP extends Environment {
	constructor(config) {
		...
		config.states.forEach((state,idx) => {
			...
			if (state.actions.length > num_actions) {
				num_actions = state.actions.length
			}
		})
		this.actions = new Array(num_actions)
	...
	perform(action) {
		if (this.current.actions.length > action) {
			...
			let state_index =  Util.sample(weights)
			this.current = this.states[state_index]
			this.reward = old.actions[action].rewards[state_index]
		} else {
...
class Gridworld extends Environment {
	...
	_move(x,y) {
		...
		let t = this.pos.get_connexion(x,y)
		if (t == undefined) {
    ...
    copy() {
        let new_env = new this.constructor(this.config)
...
class DeterministicWindyGrid extends EpisodicGrid {
	...
	_dynamics(tile) {
		let rew = config.rewards.move
		let t = tile.get_connexion(this.wind.x,this.wind.y)
		if (t == undefined) {
			rew += config.rewards.wall
		} else {
			this.pos = t
