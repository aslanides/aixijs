...
class BasicMDP extends Environment {
	constructor(config){
		...
		for (let i = 0; i < config.states.length; i++) {
			let tmp = config.states[i];
			let state = new State(i)
		...
		//Assign the actions based on the max possible from a state
		for (let i = 0; i < this.no_actions; i++) {
			this.actions[i] = i;
		}
	...
	perform(action) {
		if (this.current_state.actions.length >= (action + 1)) {
			let old_state = this.current_state
			let weights = old_state.actions[action].probabilities
