...
class BasicMDP extends Environment {
	...
	constructor(config){
		...
		for (var i = 0; i < config.states.length; i++){
			...
			var state;
			state = new State(i, tmp.reward)
			state.actions = tmp.actions
			this.states[i] = state;
		}
		this.current_state = this.states[config.initial_state];
		this.actions = []; //TODO consolidate with num_actions in agent class, as each state needs different number of actions
	}
	//Sample from the state list, and update reward and current state as necessary
	do(action){
		assert(this.current_state.actions.length >= (action - 1)) //assert the action is valid
		var weights = this.current_state.actions[action].probabilities
		...
		this.reward = this.current_state.reward
	}
	generatePercept() {
        return {
