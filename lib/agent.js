class Agent {
	constructor(options) {
		this.gamma = options.gamma; //Maybe have a vector of what kind of discount function to use at each time step as entries
		this.numActions = options.numActions;
		this.tracer = Trace;
		this.t = 0

		//arrays of which discount functions to use and when (have it as a parameter for now, predefined vectors later...)
		this.discount_array = options.discounts
		this.times = options.discount_changes
	...
	utility(e, dfr) {
		//logic to check current time and what discount to use
	//	this.discount[this.model.t] // = what discount to call
	//	return this.current_discount.eval(dfr) * e.rew
		if (this.t == this.times[this.current_discount_index]){
			this.current_discount = this.discount_array[this.current_discount_index]
			this.current_discount_index++
		}
	 	return this.current_discount.eval(dfr)
		//Math.pow(this.gamma, dfr) * e.rew; // geometric discount
	}
}
