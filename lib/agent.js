...
class BayesAgent extends Agent {
    constructor(options) {
        ...
        this.max_reward = rewards.chocolate
        this.min_reward = rewards.wall
    ...
    update(a,o,r) {
		...
		var n = this.search_tree.root.getChild(a).getChild({obs:o,rew:r})
...
class SquareKSA extends KSA {
    utility(percept,dfr) {
        if (dfr == this.horizon) {
            -1 * Math.pow(this.model.xi(percept),2)
        } else {
            return 0
        }
    }
