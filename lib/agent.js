...
class TabularAgent extends Agent {
	...
	selectAction(e) {
		...
		if (Math.random() < this.epsilon) {
			return Util.randi(this.numActions);
		}
...
class DQN extends Agent {
	constructor(options) {
		...
		this.input_space_size = options.input_space_size; // TODO must get from env
		...
		$.getJSON('lib/dqn.json', data => this.fromJSON(data));
	...
	selectAction(e) {
		...
		let a = null;
		if (Math.random() < this.epsilon) {
			a = Util.randi(this.na);
		} else {
			let out = this.forwardQ(this.net, s, false);
