class Agent {
	constructor(options) {
		...
		this.tracer = Trace;
		this.t = 0;
		this.discount = new Discount(options)
	}
	...
	utility(e, dfr) {
		return this.discount.getValue(this.t, dfr) * e.rew
	}
}
...
class BayesAgent extends Agent {
	...
	selectAction(e) {
		...
		let ret;
		ret = this.searchTree.bestAction();
		this.current_plan = this.searchTree.getPlan();
		return ret;
