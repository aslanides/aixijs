...
class Demo {
    ...
    static run(doc) {
		...
		if (config.demos[name] == undefined) {
        ...
        demo = new config.demos[name](options)
	...
	static setOptions() {
		Options.clearExplanations(document)
		document.getElementById("p_cycles").style.display="table-row"
		config.demos[document.getElementById("demo_select").value].setOptions(document)
	}
}
