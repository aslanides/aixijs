const demo = {
	...
	run(noupdate, env) {
		...
		options.agent.modelParametrization = 'goal'; // TODO default somewhere
		...
		options.agent.model = this.env.makeModel(options.agent.modelParametrization);
		...
		if (options.env.mods) {
			options.env.mods(this.env);
		}

		this.agent = new options.agent.type(options.agent);
		if (options.agent.mods) {
			options.agent.mods(this.agent);
		}
