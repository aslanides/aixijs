...
class Demo {
	constructor(dem) {
		this.gui = new UI()
		var options = this.gui.getOptions()
		if (options == null) {
			console.error("Bad options.")
			return false
		}
		this.env = new dem.env(dem.config)
		options.getEnvParams(this.env)
		this.agent = new dem.agent(options)
		// this.gui.clearExplanations()
		// for (var exp of dem.exps) {
		// 	this.gui.showExplanation(exp)
		// }
	}
	simulate(env,agent,vis,cycles) {
		this.agent = agent
		this.env = env
	...
	static init() {
        for (var o in config.demos) {
            var option = document.createElement("option")
            option.text = config.demos[o].name
