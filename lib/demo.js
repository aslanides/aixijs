const demo = {
	...
	run() {
		...
		options.modelParametrization = this.dem.modelParametrization || 'goal';
		options.modelType = this.dem.model;
		options.tracer = this.dem.tracer;
		...
		let callback = _ => {
			this.ui.show('navigation');
			this.ui.hide('cancel');
			this.ui.hide('loading');
			this.ui.show('run');
			this.vis = new this.visClass(this.env, this.trace);
			this.vis.reset();
			this.cancel = false;
		};
...
const demos = {
	...
	betaaixi_dispenser: {
		name: 'MC-AIXI-Beta vs DispenserGrid',
		agent: BayesAgent,
		env: SimpleDispenserGrid,
		model: BetaModel,
		vis: BetaBayesVis,
