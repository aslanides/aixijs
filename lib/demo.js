const demo = {
	...
	run() {
		...
		let callback = _ => {
			...
			this.vis.reset();
			this.cancel = false;
			let elapsed = Util.roundTo((performance.now() - this.t0) / 1000, 4);
			let fps = Util.roundTo(this.trace.t / elapsed, 2);
			console.log(`${this.trace.t} cycles, ${elapsed} seconds (${fps} fps)`);
		};
