...
export class Normal implements Distribution {
  ...
  prob(x: number): number {
    return (
      (1 / Math.sqrt(2 * Math.pow(this.sigma, 2) * Math.PI)) *
      Math.pow(
        Math.E,
        (-1 * Math.pow(x - this.mu, 2)) / (2 * Math.pow(this.sigma, 2))
      )
    );
  }
...
export class Beta implements Distribution {
  ...
  prob(p: number) {
    ...
    return (
      (Math.pow(p, this.alpha - 1) * Math.pow(1 - p, this.beta - 1)) / beta
    );
  }
...
export class Dirichlet implements Distribution {
  ...
  update(i: number) {
    // for performance, only one obs at a time now
    this.alphas[i]++;
    this.alphaSum++;
  }
}
