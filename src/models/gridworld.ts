...
import { Gridworld, Tile, Dispenser, Wall, Trap } from "../environments/gridworld";
...
export class DirichletGrid implements Model {
	...
	bayesUpdate(a: Action, e: Percept) {
		...
		for (let i = 0; i < this.A - 1; i++) {
			...
			this.weightQueue.append(n.y * this.size + n.x);
		...
		this.weightQueue.append(s.y * this.size + s.x);
	}
	...
	infoGain(): number {
		...
		for (let i = 0; i < this.T; i++) {
			...
			const idx = this.weightQueue.pop_back();
			stack.push(idx);
	...
	load() {
		...
		while (!this.paramQueue.isempty()) {
			const [i, j] = this.paramQueue.remove();
			for (let k = 0; k < this.T; k++) {
		...
		while (!this.weightQueue.isempty()) {
			const idx = this.weightQueue.remove();
