class TIVis extends Visualization {
	constructor(env, trace, ui) {
		...
		var counter = 0;
		for (var i = 1; i < this.plan_trace.length - 2; i++) {
		...
		for (var i = 0; i < this.pos_array.length; i++) {
			var x = this.pos_array[i].x;
			var y = this.pos_array[i].y;
			if (x > xMax) {
				xMax = x;
			}
	...
	updateAgent() {
		for (var j = 0; j < this.plan_vis.length; j++) {
			this.plan_vis[j]
				.attr('visibility', 'hidden');  //Remove?
		}

		this.plan_vis = [];
		for (var i = 0; i < this.plan_trace[this.time + 2].length; i++) {
			var planIter = this.svg.append('text')
	...
	updateEnv() {
		...
		var x = this.pos_array[this.pos_trace[this.time].current].x;
		var y = this.pos_array[this.pos_trace[this.time].current].y;
